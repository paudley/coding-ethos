...
func TestCheckPlanCompletionCommandReportsCompletedPlanWithUncheckedItems(t *testing.T) {
...
`)+"\n",
	...
	t.Setenv(configEnv, overridePath)
	mustWriteTestFile(t, "docs/SOURCE_DOCS.md", "| `pkg/` | `MODULE.md` | Package docs |\n")
	mustWriteTestFile(
		t,
		"pkg/__init__.py",
		strings.TrimSpace(`
...
func TestRuntimeIgnoreFindingsReportMissingIgnores(t *testing.T) {
	...
	if len(findings) != 1 || !strings.Contains(findings[0], ".coding-ethos/ is not ignored") {
...
func TestFormatGroupRunsManagedFormattersForMatchingFiles(t *testing.T) {
	...
	if got := runFormatGroup(Config{}, []string{"app.py", "main.go"}, false); got != 0 {
...
func TestRunHookGroupsInProcessReturnsFailure(t *testing.T) {
	...
	groups := []hookGroup{
		{Name: "pass", Commands: []hookCommand{{Name: "ok", Run: func(Config, []string) int { return 0 }}}},
		{Name: "fail", Commands: []hookCommand{{Name: "bad", Run: func(Config, []string) int { return 1 }}}},
	}
...
func TestGeminiOutcomeSummaryAndPreparedCounts(t *testing.T) {
	...
	if got := geminiPreparedFiles(prepared); !reflect.DeepEqual(got, []string{"a.py", "b.py", "c.py"}) {
...
func TestShellToolParsingHelpers(t *testing.T) {
	t.Parallel()

	finding, ok := parseShfmtSyntaxFinding("script.sh:3:4: unexpected token")
	if !ok || finding.File != "script.sh" || finding.Line != 3 || finding.Column != 4 {
		t.Fatalf("parseShfmtSyntaxFinding = %#v, %t", finding, ok)
	}

	if _, ok := parseShfmtSyntaxFinding("not shfmt"); ok {
		t.Fatal("invalid shfmt syntax line parsed")
	}

	if file, ok := shfmtDiffFile("--- scripts/run.sh.orig"); !ok || file != "scripts/run.sh" {
		t.Fatalf("shfmtDiffFile = %q, %t", file, ok)
	}

	if file, ok := shfmtDiffFile("--- /dev/null"); ok || file != "" {
		t.Fatalf("shfmtDiffFile(/dev/null) = %q, %t", file, ok)
	}
...
func TestValidateManifestCommandPassesAndFails(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("create hook runtime dir: %v", err)
	}
...
func TestBuildGeminiExecutionPlanCopiesPreparedPlans(t *testing.T) {
	...
	if !plan.DryRun || plan.Scope != geminiScopeBranch || len(plan.Checks) != 2 || plan.Checks[1].Name != "docs" {
...
func TestGeminiOutcomeCollectionFilteringAndReports(t *testing.T) {
	...
	if len(outcomes) != 1 || len(jobs) != 1 || jobs[0].CheckIndex != 0 || jobs[0].BatchIndex != 0 {
	...
	for _, format := range []string{hookOutputFormatHuman, hookOutputFormatJSON, hookOutputFormatTOON} {
		...
		if !strings.Contains(report, "critical issue") || !strings.Contains(report, "app.py") {
...
func TestGeminiBatchRequestInputsUseExplicitCacheBinding(t *testing.T) {
	...
	if prompt != "cached prompt" || dependency != "corpus" || cachedContent != "cachedContents/1" {
...
`)+"\n",
	...
	output := captureStdout(t, func() {
		if got := runGeminiCheck(Config{}, []string{"--dry-run", "--check-type", "code_ethos", "pkg/app.py"}); got != 0 {
...
func TestGeminiPromptAndCacheHelpers(t *testing.T) {
	...
	if got := geminiPromptWithInlineContent("Review\n{code_content}", "print('ok')"); !strings.Contains(got, "print('ok')") {
		t.Fatalf("inline prompt = %q", got)
	}

	if got := geminiPromptWithInlineContent("Review", "print('ok')"); !strings.Contains(got, "\n\nprint('ok')") {
...
func TestCreateAndEnsureGeminiExplicitCacheUseAPIAndPersistResult(t *testing.T) {
	...
	requests := 0
	client := &http.Client{Transport: fakeRoundTripper(func(request *http.Request) *http.Response {
		requests++
...
func TestCreateGeminiExplicitCacheReportsAPIAndShapeErrors(t *testing.T) {
	...
	errorClient := &http.Client{Transport: fakeRoundTripper(func(_ *http.Request) *http.Response {
		return testHTTPResponse(http.StatusBadRequest, `{"error":{"message":"bad request","status":"INVALID_ARGUMENT"}}`)
	})}

	_, err := createGeminiExplicitCache(errorClient, "key", "model", "content", time.Hour, "display")
	if err == nil || !strings.Contains(err.Error(), "bad request") {
		t.Fatalf("API error = %v", err)
	}

	noNameClient := &http.Client{Transport: fakeRoundTripper(func(_ *http.Request) *http.Response {
		return testHTTPResponse(http.StatusOK, `{"expireTime":"2026-01-01T00:00:00Z"}`)
	})}

	_, err = createGeminiExplicitCache(noNameClient, "key", "model", "content", time.Hour, "display")
...
func TestGenerateGeminiTextRetriesAndWritesCache(t *testing.T) {
	...
	requests := 0
	client := &http.Client{Transport: fakeRoundTripper(func(_ *http.Request) *http.Response {
		requests++
		if requests == 1 {
			return testHTTPResponse(http.StatusServiceUnavailable, `{"error":{"message":"try again"}}`)
		}
...
func TestShellAndYamlCommandsExecuteExternalToolsAndReportFindings(t *testing.T) {
	...
	mustWriteExecutable(
		t,
		filepath.Join(fakeBin, "shellcheck"),
		`#!/usr/bin/env sh
