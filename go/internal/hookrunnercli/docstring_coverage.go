...
func formatDocstringCoverageFailure(
	...
	case hookOutputFormatSARIF:
		return formatDocstringCoverageFailureSARIF(settings, stdout, stderr)
...
func formatDocstringCoverageFailureSARIF(
	settings docstringCoverageSettings,
	stdout string,
	stderr string,
) string {
	return formatHookReport(
		docstringCoverageHookReport(settings, stdout, stderr),
		hookOutputFormatSARIF,
	)
}

func docstringCoverageHookReport(
	settings docstringCoverageSettings,
	stdout string,
	stderr string,
) hookReport {
	findings := docstringCoverageFindings(settings, stdout, stderr)

	return hookReport{
		Tool:     "docstring_coverage",
		Title:    "DOCSTRING COVERAGE CHECK FAILED",
		Summary:  fmt.Sprintf("Docstring coverage is below %d%%.", settings.Threshold),
		Findings: findings,
		Guidance: []string{
...
func docstringCoverageFindings(
	settings docstringCoverageSettings,
	stdout string,
	stderr string,
) []hookFinding {
	...
	for line := range strings.SplitSeq(strings.TrimSpace(stdout), "\n") {
		missing, ok := strings.CutPrefix(strings.TrimSpace(line), "Missing: ")
		if !ok {
			continue
		}

		file, detail, _ := strings.Cut(missing, ":")
		findings = append(findings, hookFinding{
			...
			PolicyID: "python.docstring_coverage",
			SkillID:  "agent-operating-discipline",
			Message:  "Public symbol is missing a Google-style docstring.",
			Advice:   "Add a contract-level docstring or make the symbol private.",
			Detail:   detail,
		})
	}
	...
	return []hookFinding{{
		...
		PolicyID: "python.docstring_coverage",
		SkillID:  "agent-operating-discipline",
		Message:  "Docstring coverage is below the configured threshold.",
		Advice:   "Add public API docstrings until coverage reaches the threshold.",
	}}
}
