...
func TestLintToolRoutingHelpersRewriteAndBlockEvasiveShell(t *testing.T) {
	...
	rewritten, tool, rewrite, ok := rewriteLintToolCommandChain(
		"python -m ruff check pkg 2>&1",
	)
...
func TestRoutingEvidenceAndBlockDecisionHelpers(t *testing.T) {
	...
	args, redirects := splitShellRedirections(
		[]string{"status", "--short", ">", "out.txt"},
	)
	if strings.Join(args, " ") != "status --short" || len(redirects) != 1 ||
		redirects[0] != "> 'out.txt'" {
