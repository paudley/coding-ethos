...
func TestExplainPolicyWritesCELExpressionDetails(t *testing.T) {
		...
		Policies: map[string]Policy{
			"custom.no_subprocess_git": {
				...
				Suggestion: strings.Join([]string{
					"Run ordinary git commands without bypass flags or",
					"shell indirection.",
				}, " "),
	...
	output := buffer.String()
	for _, expected := range expectedExplainPolicyOutput() {
		if !strings.Contains(output, expected) {
...
func expectedExplainPolicyOutput() []string {
	return []string{
		"## CEL Expression",
		...
		expectedEventInputSchema(),
		expectedDiffInputSchema(),
		expectedChangedSymbolsInputSchema(),
		expectedDiffHunksInputSchema(),
		expectedSourceInputSchema(),
		expectedFindingsInputSchema(),
		expectedRepoInputSchema(),
		expectedReferencedFilesInputSchema(),
		"Reviewed helpers:",
...
func expectedDiffInputSchema() string {
	return "`diff: {files, changed_files, staged_files, has_changes, " +
		"hunks, added_lines, removed_lines, changed_symbols}`"
}

func expectedChangedSymbolsInputSchema() string {
	return "`changed_symbols: list({file, dir, base, ext, language, " +
		"node_kind, symbol_kind, symbol_name, symbol_path, action, " +
		"changed_lines, is_generated, is_test, original_line_count, " +
		"current_line_count, line_delta, original_nonblank_line_count, " +
		"current_nonblank_line_count, nonblank_line_delta, " +
		"line_count_grows, line_count_shrinks, nonblank_line_count_grows, " +
		"nonblank_line_count_shrinks, original_start_line, " +
		"original_end_line, current_start_line, current_end_line, " +
		"original_content_hash, current_content_hash})`"
}

func expectedDiffHunksInputSchema() string {
	return "`diff.hunks[]: {file, old_start, old_lines, new_start, " +
		"new_lines, header, added_lines, removed_lines}`"
}

func expectedSourceInputSchema() string {
	return "`source: {path, language, symbol_name, symbol_kind, " +
		"chunk_hash, line_count, changed_lines, prior_failures, " +
		"recent_remediations}`"
}

func expectedFindingsInputSchema() string {
	return "`findings: list({tool, code, message, file, language, " +
		"symbol_name, symbol_kind, chunk_hash, line, line_count, " +
		"changed_lines, severity, policy_id, skill_id, principle_ids})`"
}
...
func expectedReferencedFilesInputSchema() string {
	return "`referenced_files: list({file, dir, base, lower, exists, " +
		"is_regular, in_agent_workspace, size_bytes})`"
}
