...
func TestRealGitResolutionHelpers(t *testing.T) {
	...
	if err := os.WriteFile(self, []byte("#!/usr/bin/env sh\n"), 0o755); err != nil {
		t.Fatalf("write self: %v", err)
	}
	if err := os.WriteFile(git, []byte("#!/usr/bin/env sh\n"), 0o755); err != nil {
...
func TestResultJSONAndExitCodeError(t *testing.T) {
	...
	result := Result{Status: "blocked", Operation: "commit", Argv: []string{"git", "commit"}}
	if !result.Blocked() {
