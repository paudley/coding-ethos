...
func TestParseArgvSkipsGitGlobalOptions(t *testing.T) {
	parsed := parseArgv([]string{"-C", "/repo", "-c", "commit.gpgsign=false", "commit", "-m", "test"})
...
func policyBundleWithChangeDirPolicy(t *testing.T) policy.Bundle {
	...
	bundle.Policies["git.change_dir_flag"] = policy.Policy{
		...
		Category:        "git",
		Source:          policy.SourceRef{File: "config.yaml", Path: "git.change_dir_flag"},
		DefaultSeverity: "block",
		SupportedModes:  []string{"block", "record"},
		Message:         "git -C hides working directory context.",
		DefenseLayers:   policy.GitDefenseLayers("block", "wrapper", "block", "", ""),
		Evaluators:      []policy.Evaluator{{Kind: "argv", Name: "git.change_dir_flag"}},
	}
	bundle.Dispatch.Git["*"] = policy.GitOperationDispatch{Pre: []string{"git.change_dir_flag"}}
