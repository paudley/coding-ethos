...
func (store *Store) SARIFResults(
	...
	rows, err := store.database.QueryContext(
		...
		`SELECT
			...
			COALESCE(result.proxy_event_id, ''),
			COALESCE(result.proxy_session_id, ''),
			COALESCE(result.proxy_event_kind, ''),
			COALESCE(result.proxy_direction, ''),
			COALESCE(result.proxy_payload_kind, ''),
			COALESCE(result.proxy_trace_id, ''),
			COALESCE(result.proxy_tracking_id, ''),
			COALESCE(result.proxy_transform, ''),
