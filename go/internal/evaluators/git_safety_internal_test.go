...
func TestGitSafetyInternalParsesCommitMessagesAndGlobalArgs(t *testing.T) {
	...
	if err := os.WriteFile(messageFile, []byte("fix(core): from file\n\nbody\n"), 0o600); err != nil {
		t.Fatalf("write message file: %v", err)
	}

	messages, err := commitMessagesFromArgv(
		[]string{"GIT_DIR=.git", "git", "-C", repo, "commit", "-m", "fix(core): subject", "-msecond paragraph"},
		repo,
		nil,
	)
	if err != nil {
	...
	fileMessages, err := commitMessagesFromArgv([]string{"git", "commit", "-F", "message.txt"}, repo, nil)
	if err != nil {
	...
	stdinMessages, err := commitMessagesFromArgv([]string{"git", "commit", "--file=-"}, repo, []byte("fix(stdin): ok\n"))
	if err != nil {
...
func TestGitSafetyInternalBranchAndFlagHelpers(t *testing.T) {
	...
	if got := gitSubcommand([]string{"git", "-C", "/repo", "-c", "core.quotePath=false", "checkout", "main"}); got != "checkout" {
...
func TestLineLimitHelpersApplyOnlyToCodeGrowthPolicies(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "scripts path over limit", ext: ".txt", file: "scripts/generated", lines: 501, want: true},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got := fileExceedsLineLimit(test.ext, test.file, test.lines); got != test.want {
				t.Fatalf("fileExceedsLineLimit(%q, %q, %d) = %v, want %v", test.ext, test.file, test.lines, got, test.want)
			}
		})
	}
}
...
func TestFileSyntaxValidatorsCoverJSONAndTOML(t *testing.T) {
	...
	if err := validateJSONSyntax([]byte(`{"ok": true}`)); err != nil {
		t.Fatalf("valid json: %v", err)
	}
	if err := validateJSONSyntax([]byte(`{"bad":`)); err == nil {
		t.Fatal("invalid json should fail")
	}
	if err := validateTOMLSyntax([]byte("name = \"coding-ethos\"\n")); err != nil {
		t.Fatalf("valid toml: %v", err)
	}
	if err := validateTOMLSyntax([]byte("name = \n")); err == nil {
