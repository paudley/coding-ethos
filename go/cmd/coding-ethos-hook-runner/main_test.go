...
func TestParseShellcheckFindings(t *testing.T) {
	...
	findings := parseShellcheckFindings(
		`{"comments":[{"file":"script.sh","line":3,"column":7,"level":"warning","code":2086,"message":"Double quote to prevent globbing and word splitting."}]}`,
	)
	if len(findings) != 1 {
...
func TestParseYamllintFindings(t *testing.T) {
	...
	findings := parseYamllintFindings(
