...
class PrinciplePreset(TypedDict, total=False):
    """Optional preset fields applied to a principle by its identifier."""
...
PRINCIPLE_PRESETS: dict[str, PrinciplePreset] = {
    ...
    "fail-fast-fail-hard-overview": {
        "directive": (
            "Crash early on ambiguous startup and configuration states instead "
    ...
    "no-conditional-imports": {
        "directive": (
            "Treat required imports as hard dependencies and fail immediately "
            "if they are missing."
        ),
    ...
    "no-optional-types-for-required-dependencies": {
        "directive": (
            "Model required dependencies as non-optional and default to "
    ...
    "no-conditional-validation": {
        "directive": (
            "Run required validation unconditionally; a missing component is "
            "itself a failure."
        ),
    ...
    "no-if-available-capability-checks": {
        "directive": (
            "Validate required capabilities at startup instead of probing for "
            "them at runtime."
        ),
    ...
    "validation-at-the-gate": {
        "directive": (
            "Validate configuration, schema, and extensions during bootstrap "
    ...
    "no-inline-cli-environment-variables": {
        "directive": (
            "Route configuration through validated bootstrap paths instead of "
    ...
    "robustness-in-motion-runtime": {
        "directive": (
            "Treat startup misconfiguration and runtime transient failures as "
    ...
    "radical-visibility": {
        "directive": (
            "Log important decisions with context and instrument the system "
            "with metrics."
        ),
    ...
    "protocol-first-design": {
        "directive": (
            "Define and verify interfaces before writing or referencing "
            "implementations."
        ),
    ...
    "linting-as-code-quality-enforcement": {
        "directive": (
            "Resolve lint findings with structural fixes; suppress only with "
    ...
    "testing-as-specification": {
        "directive": (
            "Treat tests as executable behavioral contracts and update them "
            "with code changes."
        ),
    ...
    "sub-agent-delegation-and-context-isolation": {
        "directive": (
            "Use specialized agents with scoped context instead of overloading "
            "one thread."
        ),
...
AGENT_PROFILES: dict[str, dict[str, object]] = {
    "codex": {
        ...
        "notes": [
            ...
            (
                "Put durable project rules in AGENTS.md and link to deep docs "
                "instead of pasting long prose."
            ),
        ],
    },
    "claude": {
    ...
    "gemini": {
        ...
        "notes": [
            ...
            (
                "Prefer targeted reads of linked detail docs instead of "
                "loading the full corpus every time."
            ),
        ],
    },
}
...
TAG_TO_AGENT_HINTS = {
    "architecture": {
        "codex": (
            "Prefer structural refactors over tactical patches when this "
            "principle is implicated."
        ),
        "claude": (
            "Open the detailed ethos note before changing architecture or interfaces."
        ),
        "gemini": (
            "Use this rule to keep plans structurally coherent and avoid ad "
            "hoc abstractions."
        ),
    },
    "validation": {
        "codex": (
            "Treat missing prerequisites as blocking validation failures, not "
            "optional branches."
        ),
        "claude": (
            "Surface skipped checks and missing prerequisites explicitly during review."
        ),
        "gemini": (
            "Keep validation guidance strict and explicit rather than permissive."
        ),
    },
    "workflow": {
        "codex": (
            "Preserve deterministic workflows and avoid introducing alternate "
            "critical paths."
        ),
        "claude": (
            "Keep process guidance actionable and aligned with the existing "
            "repo workflow."
        ),
        "gemini": (
            "Summarize workflow expectations as concrete steps, not general advice."
        ),
    },
    "testing": {
        "codex": (
            "Treat tests as required follow-through for code changes, not "
            "optional cleanup."
        ),
        "claude": (
            "Call out testing gaps and mismatched behavioral contracts explicitly."
        ),
        "gemini": (
            "Use this principle to keep output anchored to observable behavior."
        ),
    },
    "security": {
        "codex": (
            "Prefer least-privilege and boundary-enforcing changes over soft "
            "runtime checks."
        ),
        "claude": (
            "Highlight trust-boundary and secret-handling implications during review."
        ),
        "gemini": (
            "Keep summaries explicit about security constraints and forbidden "
            "shortcuts."
        ),
    },
}
...
DEFAULT_AGENT_HINTS = {
    ...
    "gemini": (
        "Use this rule to keep high-level guidance concise, concrete, and enforceable."
    ),
}
...
def build_quick_ref(
    *,
    ...
    """Build up to three quick-reference bullets from principle text."""
    items: list[str] = []
...
def build_agent_hints(*, tags: list[str]) -> dict[str, str]:
    """Build default per-agent hints, overriding them with tag-specific ones."""
