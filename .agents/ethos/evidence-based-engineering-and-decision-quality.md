...
## Agent Hints
- `claude`: Preserve task-first planning and make evidence, assumptions, and
reversibility explicit in repo guidance.
- `codex`: Verify claims in code, tests, or measurements; batch independent work
and call out irreversible risks explicitly.
- `gemini`: Keep plans short but evidence-backed; state the trade-off and the next
validating action.
